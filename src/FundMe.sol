...
contract FundMe {
    ...
    // contract. so we set the value at compile time.
    uint internal constant MIN_DOLLAR_AMT = 10e18;

    // we want to keep track of the people who send us eth and we'll do so by adding their wallet addresses to the array below
    address[] public funders;
    // we can make a mapping to know how much each funder sent 
    mapping(address funder => uint weiSent) addressToAmountFunded;
    ...
    function fund() public payable{
        ...
        */
       require(msg.value.getConversionRate() >= MIN_DOLLAR_AMT, "A minimum of 1ETH is required");
    ...
    receive() external payable{
        ...
        if(msg.value.getConversionRate() >= MIN_DOLLAR_AMT){
            fund();
        }
    }
